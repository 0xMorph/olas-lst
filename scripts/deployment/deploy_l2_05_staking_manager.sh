...
agentId=$(jq -r '.agentId' $globals)
configHash=$(jq -r '.configHash' $globals)
...
stakingManagerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#stakingManagerAddress}
...
echo "$(jq '. += {"stakingManagerAddress":"'$stakingManagerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$stakingManagerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,address,address,address,uint256,bytes32)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $stakingManagerAddress${reset}"
